...
public class Esercizio1 {
    public static void main(String[] args) {
        ...
        Map<Customer, List<Order>> mappaOrdiniPerCliente = orders.stream()
                .collect(Collectors.groupingBy(order -> order.getCustomer()));

        mappaOrdiniPerCliente.forEach((cliente, ordini) -> {
            System.out.println("Cliente: " + cliente.getName());
            ordini.forEach(o -> System.out.println("  - Ordine ID: " + o.getId()));
        });
